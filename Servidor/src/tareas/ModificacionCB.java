	...
	private void modificarYEnviar() {
		...
		if (total - listos <= idListos.length) {
			...
			XMLAcuseReciboServer xmlAcuse = new XMLAcuseReciboServer("NO",
					"Algunos pedidos están servidos y ya no se pueden deshacer. Máximo permitido: "
							+ idListos.length);
			String acuse = xmlAcuse.xmlToString(xmlAcuse.getDOM());
			try {
				Conexion conn = new Conexion(socket);
		...
		if (total - listos <= idListos.length) {
			...
			XMLModificacionCBServer xmlModificacionesCB = new XMLModificacionCBServer(
					pedido);
			String mensaje = xmlModificacionesCB
					.xmlToString(xmlModificacionesCB.getDOM());
